		 ...
		 <!-- Step wizard -->
		  <div class="box box-default">
			...
			<div class="box-body wizard-content stepdesign">
				<form action="{{route('savebook')}}" class="tab-wizard wizard-circle" method="post">
					<!-- Step 1 -->
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
                                    ...
                                    @php $languages = json_decode($bookingav['languages'],true) @endphp
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="custom-select form-control" id="patient_type" name="patient_type">
										 @php $patient_type = json_decode($bookingav['patient_type'],true) @endphp
										@foreach (@$patient_type as $record)
										<option value="{{$record}}" >{{$record}}</option>
									@endforeach
									</select> </div>
							</div>
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select class="custom-select form-control" name="where">
										 @php $where = json_decode($bookingav['where'],true) @endphp
										@foreach (@$where as $record)
										<option value="{{$record}}" >{{$record}}</option>
									@endforeach
									  </select>
								 </div>
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select class="custom-select form-control" id="doctor_type" name="doctor_type">
										@php $doctor_type = json_decode($bookingav['doctor_type'],true) @endphp
										@foreach ($doctor_type as $v)
...
@else 
<tr><td><input type="radio" class="" name="hours" value="{{date('Y-m-d')}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($sunday_satrt) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($sunday_satrt) ) )}}</td></tr>
@endif
...
<tr><td><input type="radio" class="form-check-input" name="hours" value="{{date('Y-m-d', strtotime(' +1 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($monday_start) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($monday_start) ) )}}</td></tr>
@endfor
...
<tr><td><input type="radio" class="form-check-input" name="hours" value="{{date('Y-m-d', strtotime(' +2 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($tuesday_start) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($tuesday_start) ) )}}</td></tr>
@endfor
    </tbody>
  </table>
   <table class="table-responsive-sm">
...
<tr><td><input type="radio" class="form-check-input" name="hours" value="{{date('Y-m-d', strtotime(' +3 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($wednesday_start) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($wednesday_start) ) )}}</td></tr>
@endfor
    </tbody>
  </table>
   <table class="table-responsive-sm">
    ...
    @for($i=0;$i<=$loopthursday;$i++)
     <tr><td><input type="radio" class="form-check-input" name="hours" value="{{date('Y-m-d', strtotime(' +4 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($thursday_start) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($thursday_start) ) )}}</td></tr>
...
<tr><td><input type="radio" class="form-check-input" name="hours"value="{{date('Y-m-d', strtotime(' +5 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($friday_satrt) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($friday_satrt) ) )}}</td></tr>
@endfor
    </tbody>
  </table>
   <table class="table-responsive-sm">
...
<tr><td><input type="radio" class="form-check-input" name="hours" value="{{date('Y-m-d', strtotime(' +6 day'))}}_{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($saturday_start) ) )}}">{{date( 'H:i a', strtotime( '+'.$i.' hour' , strtotime($saturday_start) ) )}}</td></tr>
@endfor
    </tbody>
  </table>
								</div>
